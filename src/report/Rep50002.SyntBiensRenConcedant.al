...
using System.IO;
report 50002 "Synt. Biens Ren Concedant"
{
    DefaultLayout = RDLC;
    RDLCLayout = './SyntBiensRenConcedant.rdl';
    ...
    dataset
    {
        dataitem("Fixed Asset"; "Fixed Asset")
        {
            ...
            column(ReportNo_; REPORT::"Synt. Biens Ren Concedant")
            {
            }
            ...
            begin
                ...
                if (GetCessionDate() = 0D) or ((GetCessionDate() >= Deb) and (GetCessionDate() <= Fin)) then begin
                    ...
                    Montant[5] := -1 * Idsp.Dexprv("Fixed Asset"."No.", Deb, Fin);
                    Montant[6] := -1 * Idsp.Dreprv("Fixed Asset"."No.", Deb, Fin);

                    Montant[7] := -1 * Idsp.Ddeecot("Fixed Asset"."No.", Deb, Fin);
                    Montant[8] := -1 * Idsp.Dexecot("Fixed Asset"."No.", Deb, Fin);
                    Montant[9] := -1 * Idsp.Dreecot("Fixed Asset"."No.", Deb, Fin);

                    Montant[10] := Montant[1] + Montant[2] + Montant[3];
                    Montant[11] := Montant[4] + Montant[5] + Montant[6];

                    if Montant[5] < 0 then begin
                        Montant[6] += (-1 * Montant[5]);
                        Montant[5] := 0;
                    end;

                    Montant[12] := Montant[7] + Montant[8] + Montant[9];
    ...
    begin
        ...
        EnteteGen[2] := 'Provisions pour renouvellement biens provenant du concédant';
        EnteteGen[3] := 'Amortissements économiques des  biens apportés  par le concédant';

        EnteteDet[1] := 'Biens non renouvelables présents début ex.';
        EnteteDet[2] := 'entrées';
        EnteteDet[3] := 'Biens sortis sur l exercice';
        EnteteDet[4] := 'Biens non renouV. présents fin de l.exercice';

        EnteteDet[5] := 'Provisions en début d exercice';
        EnteteDet[6] := 'Dotations de l exercice';
        EnteteDet[7] := 'Reprises de l exercice';
        EnteteDet[8] := 'Provisions en fin d exercice';

        EnteteDet[9] := 'Amortissements en début d exercice';
        EnteteDet[10] := 'Dotations de l exercice';
        EnteteDet[11] := 'Reprises de l exercice';
        EnteteDet[12] := 'Amortissements en Fin d.exercice';
    end;
    ...
    var
        ...
        Text001: Label 'Trial Balance', Comment = 'FRA="Synthèse des biens renouvelables apportés par le concedant"';
        ...
        EnteteDet: array[12] of Text[100];
    ...
    begin
        ...
        TempExcelBuf.AddInfoColumn(REPORT::"Synt. Biens Ren Concedant", false, false, false, false, '', 0);
        TempExcelBuf.NewRow();
    ...
    begin
        ...
        TempExcelBuf.AddColumn(EnteteDet[10], false, '', true, false, false, '', 0);
        TempExcelBuf.AddColumn(EnteteDet[11], false, '', true, false, false, '', 0);
        TempExcelBuf.AddColumn(EnteteDet[12], false, '', true, false, false, '', 0);
    ...
    begin
        ...
        //TODO: GiveUserControl n'exite pas dans TempExcelBuf
