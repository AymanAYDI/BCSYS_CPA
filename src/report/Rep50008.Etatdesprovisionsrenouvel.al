...
report 50008 "Etat des provisions renouvel"
{
    ...
    dataset
    {
        dataitem("Fixed Asset"; "Fixed Asset")
        {
            ...
            column(ReportNo_; REPORT::"Etat des provisions renouvel")
            {
            }
                    ...
                    case "In Conncession By" of
                        "In Conncession By"::Concédant:
                            begin
                                ...
                                Montant[2] := -1 * Idsp.Dfeprv("Fixed Asset"."No.", Deb, Fin);

                                Montant[7] := -1 * Idsp.Dexprv("Fixed Asset"."No.", Deb, Fin);
                                Montant[8] := -1 * Idsp.Dreprv("Fixed Asset"."No.", Deb, Fin);
                            end;

                        "In Conncession By"::Concessionaire:
                            begin
                                Montant[3] := -1 * Idsp.Ddeprv("Fixed Asset"."No.", Deb, Fin);
                                Montant[4] := -1 * Idsp.Dfeprv("Fixed Asset"."No.", Deb, Fin);

                                Montant[9] := -1 * Idsp.Dexprv("Fixed Asset"."No.", Deb, Fin);
                                Montant[10] := -1 * Idsp.Dreprv("Fixed Asset"."No.", Deb, Fin);
                            end;
                    end;
                    if Montant[7] < 0 then begin
                        Montant[8] := Montant[8] + ABS(Montant[7]);
                        Montant[7] := 0;
                    end;
    ...
    trigger OnPreReport()
    begin
        EnteteGen[1] := 'BIENS RENOUVELABLES';
        EnteteGen[2] := 'TOTAL GENERAL';
        EnteteGen[3] := 'BIENS RENOUVELABLES';

        EnteteInt[1] := 'CONCEDANT';
        EnteteInt[2] := 'CONCESIONNAIRE';

        EnteteDet[1] := 'Provisions en début d.exercice';
        EnteteDet[2] := 'Provisions en fin   d.exercice';

        EnteteDet[3] := 'Dotations de l.exercice';
        EnteteDet[4] := 'Reprises  de l.exercice';
    end;
    ...
    var
        ...
        Text001: Label 'Etat des provisions pour renouvellement', Comment = 'FRA="Etat des provisions pour renouvellement"';
    ...
    begin
        ...
        TempExcelBuf.AddInfoColumn(REPORT::"Etat des provisions renouvel", false, false, false, false, '', 0);
        TempExcelBuf.NewRow();
