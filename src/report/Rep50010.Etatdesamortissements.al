...
report 50010 "Etat des amortissements"
{
    ...
    dataset
    {
        dataitem("Fixed Asset"; "Fixed Asset")
        {
            ...
            column(ReportNo_; REPORT::"Etat des amortissements")
            {
            }
                ...
                if (GetCessionDate() = 0D) or ((GetCessionDate() >= Deb) and (GetCessionDate() <= Fin)) then begin
                    case "In Conncession By" of
                        "In Conncession By"::Concédant:
                            begin
                                if Renewable = Renewable::Oui then begin
                                    Montant[1] := -1 * Idsp.Ddeecot("Fixed Asset"."No.", Deb, Fin);
                                    Montant[2] := -1 * (Idsp.Ddeecot("Fixed Asset"."No.", Deb, Fin)
                                                       + Idsp.Dexecot("Fixed Asset"."No.", Deb, Fin)
                                                       + Idsp.Dreecot("Fixed Asset"."No.", Deb, Fin));
                                end;
                                if Renewable = Renewable::Non then begin
                                    Montant[3] := -1 * Idsp.Ddeecot("Fixed Asset"."No.", Deb, Fin);
                        ...
                        "In Conncession By"::Concessionaire:
                            begin
                                if Renewable = Renewable::Oui then begin
                                    Montant[5] := -1 * Idsp.Ddeecot("Fixed Asset"."No.", Deb, Fin);
                                    Montant[6] := -1 * (Idsp.Ddeecot("Fixed Asset"."No.", Deb, Fin)
                                                       + Idsp.Dexecot("Fixed Asset"."No.", Deb, Fin)
                                                       - Idsp.Dreecot("Fixed Asset"."No.", Deb, Fin));
                                end;
                                if Renewable = Renewable::Non then begin
    ...
    begin
        ...
        EnteteDet[1] := 'Amortissements en début d.exercice';
        EnteteDet[2] := 'Amortissements  à la fin de l.exercice';
    end;
    ...
    var
        ...
        Text001: Label 'Etat des amortissements', Comment = 'FRA="Etat des amortissements"';
    ...
    begin
        ...
        TempExcelBuf.AddInfoColumn(REPORT::"Etat des amortissements", false, false, false, false, '', 0);
        TempExcelBuf.NewRow();
