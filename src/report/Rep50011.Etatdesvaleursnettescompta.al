...
report 50011 "Etat des valeurs nettes compta"
{
    ...
    dataset
    {
        dataitem("Fixed Asset"; "Fixed Asset")
        {
            ...
            begin
                ...
                CALCFIELDS("Book Value PROV");

                if ("Book Value" <> 0) and
                  ((GetCessionDate() = 0D) or ((GetCessionDate() >= Deb) and (GetCessionDate() <= Fin))) then begin
                    case "In Conncession By" of
                        "In Conncession By"::Concédant:
                            begin
                                if Renewable = Renewable::Oui then
                                    Montant[1] := "Book Value" + "Book Value PROV";
                                if Renewable = Renewable::Non then
                                    Montant[2] := "Book Value" + "Book Value PROV";
                            end;

                        "In Conncession By"::Concessionaire:
                            begin
                                if Renewable = Renewable::Oui then
                                    Montant[3] := "Book Value" + "Book Value PROV";
                                if Renewable = Renewable::Non then
                                    Montant[4] := "Book Value" + "Book Value PROV";
                            end;
                    end;
    ...
    begin
        ...
        EnteteInt[3] := 'Total 675';
        EnteteInt[4] := 'Total 229503';

        EnteteDet[1] := 'Valeur nette comptable';
    end;
    ...
    begin
        ...
        TempExcelBuf.AddInfoColumn(REPORT::"Etat des valeurs nettes compta", false, false, false, false, '', 0);
        TempExcelBuf.NewRow();
