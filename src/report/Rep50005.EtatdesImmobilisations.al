...
report 50005 "Etat des Immobilisations"
{
    ...
    dataset
    {
        dataitem("Fixed Asset"; "Fixed Asset")
        {
            ...
            column(ReportNo_; REPORT::"Etat des Immobilisations")
            {
            }
            ...
            begin
                ...
                if FASubclass.Code <> "FA Subclass Code" then
                    if not FASubclass.GET("FA Subclass Code") then
                        FASubclass.INIT();
                NbDeprYear := "Fixed Asset".GetEcoDeprecYear();

                if (GetCessionDate() = 0D) or ((GetCessionDate() >= Deb) and (GetCessionDate() <= Fin)) then begin

                    if "In Conncession By" = "In Conncession By"::Concédant then begin
                        if Renewable = Renewable::Oui then begin
                            Montant[1] := Idsp.Vdeeco("Fixed Asset"."No.", Deb, Fin);
                            Montant[2] := Idsp.Vdeeco("Fixed Asset"."No.", Deb, Fin)
                              + Idsp.Veeeco("Fixed Asset"."No.", Deb, Fin)
                              - Idsp.Veseco("Fixed Asset"."No.", Deb, Fin);
                        end;
                        if Renewable = Renewable::Non then begin
                            Montant[3] := Idsp.Vdeeco("Fixed Asset"."No.", Deb, Fin);
                            Montant[4] := Idsp.Vdeeco("Fixed Asset"."No.", Deb, Fin)
                              + Idsp.Veeeco("Fixed Asset"."No.", Deb, Fin)
                              - Idsp.Veseco("Fixed Asset"."No.", Deb, Fin);
                        end;
                    end;

                    if "In Conncession By" = "In Conncession By"::Concessionaire then begin
                        if Renewable = Renewable::Oui then begin
                            Montant[5] := Idsp.Vdeeco("Fixed Asset"."No.", Deb, Fin);
                            Montant[6] := Idsp.Vdeeco("Fixed Asset"."No.", Deb, Fin)
                              + Idsp.Veeeco("Fixed Asset"."No.", Deb, Fin)
                              - Idsp.Veseco("Fixed Asset"."No.", Deb, Fin);
                        end;
                        if Renewable = Renewable::Non then begin
                            Montant[7] := Idsp.Vdeeco("Fixed Asset"."No.", Deb, Fin);
                            Montant[8] := Idsp.Vdeeco("Fixed Asset"."No.", Deb, Fin)
                              + Idsp.Veeeco("Fixed Asset"."No.", Deb, Fin)
                              - Idsp.Veseco("Fixed Asset"."No.", Deb, Fin);
                        end;
                    end;
    ...
    labels
    {
        ...
        lblNbDeprYear = 'Nb. années amort.', Comment = 'FRA="Nb. années amort."';
        lblDateAcqui = 'Date Acquisit.', Comment = 'FRA="Date Acquisit."';
    }
    ...
    begin
        ...
        EnteteDet[1] := 'Biens présents au début de l.exercice';
        EnteteDet[2] := 'Biens présents à la fin de l.exercice';
    end;
    ...
    var
        ...
        Text001: Label 'Etat des Immobilisations', Comment = 'FRA="Etat des Immobilisations"';
    ...
    begin
        ...
        TempExcelBuf.AddInfoColumn(REPORT::"Etat des Immobilisations", false, false, false, false, '', 0);
        TempExcelBuf.NewRow();
